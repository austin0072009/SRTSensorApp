...
class FileHelper {
    ...
    fun save(filecontent: Vector<FloatArray>) {
        //这里我们使用私有模式,创建出来的文件只能被本应用访问,还会覆盖原文件哦
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            val filename = Environment.getExternalStorageDirectory().canonicalPath + "/" + "SensorDataRecord"
            val output = FileOutputStream(filename)
            val jsonArray = JSONArray()
            for (i in filecontent.indices) {
                val json = JSONObject()
                json.put("Acceleration_X", filecontent[i][0])
                json.put("Acceleration_Y", filecontent[i][1])
                json.put("Acceleration_Z", filecontent[i][2])
                jsonArray.put(json)
            }
